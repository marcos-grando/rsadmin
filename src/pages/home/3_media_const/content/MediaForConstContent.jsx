...
import MediaForConstContentGraphic from "./MediaForConstContentGraphic";

function MediaForConstContent({ data, style }) {

    const maxValue = Math.max(...data.map(item => item.max)) + 50;

    // retorna o xLabel com "..." caso > 8 caracteres;
    const textTruncate = (text, max = 9) => {
        if (!text) return "";
            ...
            <div className={style.xLabels}>
                {data.map((value, i) => (
                    <div key={i} className={style.xLabel}>
                        <p title={value.constName}>{textTruncate(value.constName)}</p>
                    </div>
                ))}
            </div>

            <MediaForConstContentGraphic data={data} maxValue={maxValue} style={style} />

        </div>
    )
}

export default MediaForConstContent;